...
def load_efficientnet(model, checkpoint_file, model_name):
    ...
    conversion_table_for_weights_outside_blocks = {
        ...
        model._conv_stem.conv.weight: 'stem/conv2d/kernel',  # [3, 3, 3, 32]),
        ...
        model._conv_head.conv.weight: 'head/conv2d/kernel',  # [1, 1, 320, 1280]),
    ...
    conversion_table_for_first_block = {
        ...
        model._blocks[0]._project_conv.conv.weight: 'blocks_0/conv2d/kernel',  # 1, 1, 32, 16]),
        model._blocks[0]._depthwise_conv.conv.weight: 'blocks_0/depthwise_conv2d/depthwise_kernel',  # [3, 3, 32, 1]),
        model._blocks[0]._se_reduce.conv.bias: 'blocks_0/se/conv2d/bias',  # , [8]),
        model._blocks[0]._se_reduce.conv.weight: 'blocks_0/se/conv2d/kernel',  # , [1, 1, 32, 8]),
        model._blocks[0]._se_expand.conv.bias: 'blocks_0/se/conv2d_1/bias',  # , [32]),
        model._blocks[0]._se_expand.conv.weight: 'blocks_0/se/conv2d_1/kernel',  # , [1, 1, 8, 32]),
    ...
    for i in range(len(model._blocks)):
        ...
        is_first_block = '_expand_conv.conv.weight' not in [n for n, p in model._blocks[i].named_parameters()]
        ...
        if is_first_block:
            conversion_table_block = {
                ...
                model._blocks[i]._project_conv.conv.weight: 'blocks_' + str(i) + '/conv2d/kernel',  # 1, 1, 32, 16]),
                model._blocks[i]._depthwise_conv.conv.weight: 'blocks_' + str(i) + '/depthwise_conv2d/depthwise_kernel',
                # [3, 3, 32, 1]),
                model._blocks[i]._se_reduce.conv.bias: 'blocks_' + str(i) + '/se/conv2d/bias',  # , [8]),
                model._blocks[i]._se_reduce.conv.weight: 'blocks_' + str(i) + '/se/conv2d/kernel',  # , [1, 1, 32, 8]),
                model._blocks[i]._se_expand.conv.bias: 'blocks_' + str(i) + '/se/conv2d_1/bias',  # , [32]),
                model._blocks[i]._se_expand.conv.weight: 'blocks_' + str(i) + '/se/conv2d_1/kernel',  # , [1, 1, 8, 32]),
            ...
            conversion_table_block = {
                ...
                model._blocks[i]._expand_conv.conv.weight:       'blocks_' + str(i) + '/conv2d/kernel',
                model._blocks[i]._project_conv.conv.weight:      'blocks_' + str(i) + '/conv2d_1/kernel',
                model._blocks[i]._depthwise_conv.conv.weight:    'blocks_' + str(i) + '/depthwise_conv2d/depthwise_kernel',
                model._blocks[i]._se_reduce.conv.bias:           'blocks_' + str(i) + '/se/conv2d/bias',
                model._blocks[i]._se_reduce.conv.weight:         'blocks_' + str(i) + '/se/conv2d/kernel',
                model._blocks[i]._se_expand.conv.bias:           'blocks_' + str(i) + '/se/conv2d_1/bias',
                model._blocks[i]._se_expand.conv.weight:         'blocks_' + str(i) + '/se/conv2d_1/kernel',
...
def load_and_save_temporary_tensorflow_model(model_name, model_ckpt, example_img= 'examples/simple/img.jpg'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tf_checkpoint', type=str, default='pretrained_tf/efficientnet-b0/',
